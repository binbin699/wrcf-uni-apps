  ...
  () => {
    ...
    const googleLogin = async (): Promise<boolean> => {
      ...
      try {
        ...
        loginError.value = isRequestHandledError(error)
          ? ''
          : error.message || $t('login.google_login_failed');
        return false;
      } finally {
    ...
    const wxAppLogin = async (): Promise<boolean> => {
      ...
      try {
        ...
        loginError.value = isRequestHandledError(error) ? '' : errMsg || $t('login.wx_login_failed');
        return false;
      } finally {
        isLoading.value = false;
    ...
    const appleLogin = async (): Promise<boolean> => {
      ...
      try {
        ...
        loginError.value = isRequestHandledError(error)
          ? ''
          : errMsg || $t('login.apple_login_failed');
        return false;
      } finally {
        isLoading.value = false;
      }
    ...
    const passwordLogin = async (loginForm: IPasswordLoginForm): Promise<boolean> => {
      ...
      try {
        ...
        loginError.value = isRequestHandledError(error) ? '' : error.message || $t('login.login_failed');
        return false;
      } finally {
        isLoading.value = false;
      }
    ...
    const register = async (registerForm: IPasswordRegisterForm): Promise<boolean> => {
      ...
      try {
        ...
        loginError.value = isRequestHandledError(error)
          ? ''
          : error.message || $t('login.register_failed');
        return false;
      } finally {
        isLoading.value = false;
      }
    ...
    const guestLogin = async (): Promise<boolean> => {
      ...
      try {
        ...
        loginError.value = isRequestHandledError(error)
          ? ''
          : error.message || $t('login.guest_login_failed');
        return false;
      } finally {
        isLoading.value = false;
      }
