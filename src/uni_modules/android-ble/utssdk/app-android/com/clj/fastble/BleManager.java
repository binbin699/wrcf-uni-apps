...
import com.clj.fastble.entity.UserInfoEntity;
...
public class BleManager {
     ...
     * @param callback
     */
    public void bindWatch(BleDevice device,String account,String password,BleWriteCallback callback){
        ByteBuffer nByteBuffer = ByteBuffer.allocate(5024);
        nByteBuffer.put(new byte[]{0x12, 0x12, 0x12, 0x12, 0x12});
        try {
            String json = new UserInfoEntity(1,account,password).toJson();
            byte[] sendBt = json.getBytes("utf-8");
            nByteBuffer.putInt(sendBt.length);
            nByteBuffer.put(sendBt);
            nByteBuffer.flip();
            byte[] bytes = new byte[nByteBuffer.remaining()];
            nByteBuffer.get(bytes);
            write(device, "0000fff0-0000-1000-8000-00805f9b34fb", "0000fff2-0000-1000-8000-00805f9b34fb", bytes, callback);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     * @param callback
     */
    public void writeNotication(BleDevice device, MessageEntity data, BleWriteCallback callback){
        ByteBuffer nByteBuffer = ByteBuffer.allocate(5024);
        nByteBuffer.put(new byte[]{0x12, 0x12, 0x12, 0x12, 0x12});
        try {
            String json = new UserInfoEntity((byte) 2,data).toJson();
            byte[] sendBt = json.getBytes("utf-8");
            nByteBuffer.putInt(sendBt.length);
            nByteBuffer.put(sendBt);
            nByteBuffer.flip();
            byte[] bytes = new byte[nByteBuffer.remaining()];
            nByteBuffer.get(bytes);
            write(device, "0000fff0-0000-1000-8000-00805f9b34fb", "0000fff2-0000-1000-8000-00805f9b34fb", bytes,true, callback);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void write(BleDevice bleDevice,
                      ...
                      boolean split,
                      boolean sendNextWhenLastSuccess,
                      long intervalBetweenTwoPackage,int writeType,
                      BleWriteCallback callback) {
        ...
        if (bleBluetooth == null) {
            ...
            if (split && data.length > getSplitWriteNum()) {
                new SplitWriter().splitWrite(bleBluetooth, uuid_service, uuid_write, data,
                        sendNextWhenLastSuccess, intervalBetweenTwoPackage, callback,writeType);
            } else {
                bleBluetooth.newBleConnector()
                        .withUUIDString(uuid_service, uuid_write)
